"""The compliance-check prompt sent to Gemini for the "Analyse with AI" step.

Unlike a per-clause check, this looks at the worker's full situation against
the combined set of retrieved Labour Act sections, and returns findings shaped
consistently with the deterministic_findings format — so the frontend can render
both together without special-casing. Findings must cite only the retrieved
sections (the schema validator enforces this to block hallucinated citations).
"""
...
5. Write "plain_explanation_en" in plain, non-legal English a worker can understand.
6. Write "plain_explanation_ne" in plain, simple Nepali a worker can understand (same meaning as the English).
7. Write "suggested_action_en" as a concrete next step the worker could take, in English.
8. Write "suggested_action_ne" as the same concrete next step, in Nepali.
9. Do not provide legal advice beyond what the retrieved legal text supports.
